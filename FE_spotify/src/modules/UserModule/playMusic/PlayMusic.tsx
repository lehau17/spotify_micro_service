...
import { useGetSongQuery } from "@/query/song";
...
export default function PlayMusic() {
  ...
  const [dataMusic, setDataMusic] = useState<SongDto>();
  ...
  const {
    ...
    isError,
    refetch,
  } = useGetSongQuery(Number(idMusic));
  ...
  useEffect(() => {
    ...
    setDataMusic(dataSong?.data.data);
  }, [idMusic, isLoading, isError]);

  return (
    dataMusic && (
      <div className="container mx-auto play-music fixed bottom-0 left-0 right-0 z-50 bg-black text-white p-3 border-2 border-slate-500">
